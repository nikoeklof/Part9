import { NewPatient, Gender, BaseEntry, HealthCheckRating, Entry, Diagnose } from '../types/types';
...
const parseArrayStringCodes = (data: any): Array<Diagnose['code']> => {
    ...
    const errorMessage = '"diagnosisCodes" is an array of codes as string';

    try {
        const dataCodes: Array<Diagnose['code']> = typeof data === 'object' ? data : JSON.parse(data);
        if (!Array.isArray(dataCodes)) throw new Error(errorMessage);
